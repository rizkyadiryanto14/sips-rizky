...
$(document).ready(function() {
            ...
            "columns": [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        ...
                        return '\
                            <div class="text-center">\
                                <button class="btn btn-sm btn-info btn-detail" data-id="' + data.id +
                            '" data-persetujuan_kaprodi="' + data.persetujuan_kaprodi +
                            '" data-persetujuan_pembimbing="' + data.persetujuan_pembimbing +
                            '" data-komentar_kaprodi="' + data.komentar_kaprodi +
                            '" data-komentar_pembimbing="' + data.komentar_pembimbing +
                            '" data-sk_tim="' + data.sk_tim + '" type="button" data-toggle="modal" data-target="#detail">\
                                    <i class="fa fa-pen"></i>\
                                </button>\
                                ' + hapus + '\
                            </div>\
    ...
    $(document).on('click', 'button.btn-detail', function() {
        persetujuan_kaprodi = ($(this).data('persetujuan_kaprodi') == '1') ?
            '<span class="badge badge-success">Disetujui</span>' : $(this).data('komentar_kaprodi');
        persetujuan_pembimbing = ($(this).data('persetujuan_pembimbing') == '1') ?
            '<span class="badge badge-success">Disetujui</span>' : $(this).data('komentar_pembimbing');
        sk_tim = (!$(this).data('sk_tim')) ? '<span class="badge badge-danger">Belum Tersedia</span>' :
            '<a class="btn btn-primary btn-sm" href="' + base_url + 'cdn/vendor/sk_tim/' + $(this).data(
                'sk_tim') + '">' + $(this).data('sk_tim') + '</a>'
