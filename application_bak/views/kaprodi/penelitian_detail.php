    ...
    function show() {
        call('api/penelitian/details/' + penelitian_id).done(function(res) {
            if (res.error == true) {
                ...
                $('.bukti').html('<a href="' + base_url + 'cdn/vendor/penelitian/' + res.data.bukti +
                    '">' + res.data.bukti + '</a>');
                $('.file_seminar').html((res.data.file_seminar) ? `<a href="` + base_url +
                    `cdn/vendor/penelitian/file_seminar/` + res.data.file_seminar + `">` + res.data
                    .file_seminar + `</a>` :
                    '<input type="file" accept="application/pdf" class="form-control" name="pilih-sk_tim">'
                );
                $('.sk_tim').html((res.data.sk_tim) ? `<a href="` + base_url +
                    `cdn/vendor/penelitian/sk_tim/` + res.data.sk_tim + `">` + res.data.sk_tim +
                    `</a>` :
                    '<input type="file" accept="application/pdf" class="form-control" name="pilih-sk_tim">'
                );
                $('.bukti_konsultasi').html((res.data.bukti_konsultasi) ? `<a href="` + base_url +
                    `cdn/vendor/penelitian/bukti_konsultasi/` + res.data.bukti_konsultasi + `">` +
                    res.data.bukti_konsultasi + `</a>` :
                    '<input type="file" accept="application/pdf" class="form-control" name="pilih-sk_tim">'
                );
